...
import type { EntityId, OwnedPropertyState, ServiceTask, TaskType } from "@/types";
...
export function activeTaskForEmployee(prop: OwnedPropertyState, employeeId: EntityId): ServiceTask | undefined {
  return prop.tasks.find((t) => t.assignedEmployeeId === employeeId && ACTIVE_STATUSES.includes(t.status));
}

export function activeTaskForEquipment(prop: OwnedPropertyState, equipmentId: EntityId): ServiceTask | undefined {
  return prop.tasks.find((t) => t.equipmentId === equipmentId && ACTIVE_STATUSES.includes(t.status));
}

export function activeTaskForAttraction(prop: OwnedPropertyState, attractionId: EntityId): ServiceTask | undefined {
  return prop.tasks.find((t) => t.attractionId === attractionId && ACTIVE_STATUSES.includes(t.status));
}

function targetLabel(prop: OwnedPropertyState, task: ServiceTask): string {
  if (ORDER_TASK_TYPES.has(task.type) && task.orderId) {
    const order = prop.orders.find((o) => o.id === task.orderId);
    const customer = task.customerId ? prop.customers.find((c) => c.id === task.customerId) : undefined;
    if (order) {
  ...
  if (task.customerId) {
    const c = prop.customers.find((c) => c.id === task.customerId);
    if (c) return `${c.firstName} ${c.lastName}`;
  }
  if (task.equipmentId) {
    const e = prop.equipment.find((e) => e.id === task.equipmentId);
    if (e) return e.name;
  }
  if (task.attractionId) {
    const a = prop.attractions.find((a) => a.id === task.attractionId);
    if (a) return a.name;
  }
  return "";
}
...
export function describeTask(prop: OwnedPropertyState, task: ServiceTask): string {
  const verb = TASK_TYPE_LABEL[task.type] ?? task.type;
  const target = targetLabel(prop, task);
