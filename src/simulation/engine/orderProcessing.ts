...
import type { Customer, Employee, GameState, InventoryItem, MenuListing, Order, OwnedPropertyState, Product, ServiceTask } from "@/types";
import { logActivity } from "./activityLogger";

function getActiveMenu(prop: OwnedPropertyState): MenuListing[] {
  return prop.menu.filter((listing) => listing.isActive);
}

/** Whether the player owns *usable* equipment matching the product's recipe requirement, if any (Master Plan Sections 33-34). */
export function hasRequiredEquipment(prop: OwnedPropertyState, productId: string): boolean {
  const recipe = getRecipeForProduct(productId);
  if (!recipe.requiredEquipmentCategory) return true;
  return prop.equipment.some((e) => e.category === recipe.requiredEquipmentCategory && isEquipmentUsable(e));
}
...
function selectProductForCustomer(
  ...
  const activeMenu = getActiveMenu(prop);
  const affordable = activeMenu.filter((listing) => {
    if (effectivePrice(state, prop, listing.productId, listing.price) > customer.spendingBudget - customerSpentSoFar(prop, customer))
      return false;
    if (!allowAlcohol && ALCOHOLIC_PRODUCT_IDS.has(listing.productId)) return false;
    if (!hasRequiredEquipment(prop, listing.productId)) return false;
    return true;
  });
...
function openOrReuseTab(state: GameState, prop: OwnedPropertyState, customer: Customer): string {
...
export function ensureOrderTasks(state: GameState, prop: OwnedPropertyState, rng: SeededRandom, bus: EventBus): void {
  const hasTaskFor = (customerId: string, type: ServiceTask["type"]) =>
    prop.tasks.some((t) => t.customerId === customerId && t.type === type && t.status !== "complete" && t.status !== "cancelled");
  ...
  for (const customer of prop.customers) {
    ...
    const watchingTv = (customer.status === "consuming" || customer.status === "deciding_next_order") && hasOperationalTv(prop);
    const wantsToOrder =
...
export function consumeInventoryForOrder(prop: OwnedPropertyState, order: Order, employee: Employee): PrepResult {
  ...
  const waste =
    ...
    equipmentWasteMultiplier(prop, recipe.requiredEquipmentCategory);
  ...
  const consumption = recipe.ingredients.map((ing) => ({
    item: prop.inventory.find((i) => i.id === ing.inventoryItemId) as InventoryItem | undefined,
...
export function baseCostCentsForProduct(prop: OwnedPropertyState, productId: string): number {
  const recipe = getRecipeForProduct(productId);
...
export function buildOrder(state: GameState, prop: OwnedPropertyState, customer: Customer, listing: MenuListing): Order {
  const tabId = openOrReuseTab(state, prop, customer);
  const order: Order = {
    id: createId("order"),
    customerId: customer.id,
