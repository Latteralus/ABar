...
import type { Employee, GameState, ServiceTask } from "@/types";
...
export function ensureAttractionTasks(state: GameState, bus: EventBus): void {
  ...
  const hasPendingTaskFor = (attractionId: string, type: "repair_attraction" | "clean_attraction") =>
    state.tasks.some((t) => t.type === type && t.attractionId === attractionId && t.status !== "complete" && t.status !== "cancelled");
...
export function handleCleanAttractionComplete(state: GameState, bus: EventBus, task: ServiceTask, employee: Employee): void {
...
export function handleRepairAttractionComplete(state: GameState, bus: EventBus, task: ServiceTask, employee: Employee): void {
  const attraction = state.attractions.find((a) => a.id === task.attractionId);
  if (!attraction) return;
...
export function resolveDueAttractionContractRepairs(state: GameState, bus: EventBus): void {
  for (const attraction of state.attractions) {
