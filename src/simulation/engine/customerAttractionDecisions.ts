...
import type { Customer, EntityId, GameState, OwnedPropertyState } from "@/types";
...
export function processAttractionDecisions(state: GameState, prop: OwnedPropertyState, rng: SeededRandom, bus: EventBus): void {
  ...
  for (const customer of prop.customers) {
    ...
    if (customer.groupId) {
      ...
      const group = prop.customerGroups.find((g) => g.id === customer.groupId);
      if (!group) continue;
      const members = group.memberIds.map((id) => prop.customers.find((c) => c.id === id)).filter((c): c is Customer => !!c);
      if (members.length === 0 || !members.every(isEligible)) continue; // whole group must be free to decide together
      considerAttractions(state, prop, rng, bus, members, customer.groupId);
    } else {
      considerAttractions(state, prop, rng, bus, [customer], null);
    }
  }
}
