...
import type { Equipment, EquipmentCategory, GameState } from "@/types";
...
function bestUsableCondition(state: GameState, category: EquipmentCategory): number | undefined {
...
function degradedSeverity(state: GameState, category: EquipmentCategory): number {
  const best = bestUsableCondition(state, category);
...
export function equipmentWasteMultiplier(state: GameState, category?: EquipmentCategory): number {
  if (!category) return 1;
  return 1 + degradedSeverity(state, category) * MAINTENANCE_CONFIG.wastePenaltyAtZeroCondition;
}

/** Extra wear from actually using a piece of equipment (e.g. one prepare_drink completion). */
export function decayEquipmentOnUse(state: GameState, category: EquipmentCategory): void {
...
export function applyDailyEquipmentWear(state: GameState, bus: EventBus): void {
  for (const eq of state.equipment) {
...
export function processEquipmentWear(state: GameState, rng: SeededRandom, bus: EventBus): void {
  for (const eq of state.equipment) {
    if (eq.currentStatus !== "degraded") continue;
    const chance = degradedSeverity(state, eq.category) * MAINTENANCE_CONFIG.maxPerMinuteBreakdownChance;
...
export function ensureMaintenanceTasks(state: GameState, bus: EventBus): void {
...
export function resolveDueContractRepairs(state: GameState, bus: EventBus): void {
  for (const eq of state.equipment) {
