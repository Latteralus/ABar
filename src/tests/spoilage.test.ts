...
describe("applySpoilage", () => {
  ...
  it("does not spoil stock that is still within shelf life", () => {
    ...
    applySpoilage(state, prop, bus, property);
  ...
  it("shortens effective shelf life for items in an over-capacity storage pool", () => {
    ...
    prop.equipment = prop.equipment.filter((e) => e.category !== "refrigerator");
...
describe("getStorageUsage", () => {
  it("adds owned, usable storage_shelving capacity to the general pool", () => {
    const { state, prop, bus, property } = setup();
    const before = getStorageUsage(prop, property).general.capacity;

    commandService.purchaseEquipment(state, bus, "equip-storage-shelving");
    const after = getStorageUsage(prop, property).general.capacity;

    expect(after).toBe(before + 250);
  });

  it("does not count a failed refrigerator's capacity toward the refrigerated pool", () => {
    const { prop, property } = setup();
    const fridge = prop.equipment.find((e) => e.category === "refrigerator")!;
    const before = getStorageUsage(prop, property).refrigerated.capacity;
    fridge.currentStatus = "failed";

    expect(getStorageUsage(prop, property).refrigerated.capacity).toBe(before - (fridge.capacity ?? 0));
  });
});
